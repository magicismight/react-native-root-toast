...
class Field extends Component {
  ...
  state = {
    value: this.props.options[this.keys[0]],
    text: `Toast.${this.props.name}s.${this.keys[0]}`,
    picked: this.keys[0],
    picker: false,
  };
  ...
  pickerChange = value => {
    ...
    this.setState({
      ...
      picked: value,
      text: value
        ? `Toast.${this.props.name}s.${value}`
        : `Custom ${this.props.name}`,
    });
      ...
      <View style={styles.field}>
        ...
        <Modal style={styles.pickerModal} visible={this.state.picker}>
          <Picker
            ...
            <Picker.Item label={`Custom ${this.props.name}`} value={false} />
          </Picker>
          <TouchableOpacity
